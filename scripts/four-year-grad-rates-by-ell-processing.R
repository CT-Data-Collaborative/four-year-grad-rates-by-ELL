...
##################################################################
#
# Processing Script for Four-Year-Grad-Rates-by-ELL
...
four_yr_gr <- merge(four_yr_gr_grad, four_yr_gr_nongrad, by = c("District", "English Learner Status", "Total Cohort Count", "Year"))
...
backfill_years <- expand.grid(
  ...
  `English Learner Status` = unique(four_yr_gr_fips$`English Learner Status`),
  `Year` = years
)
...
complete_four_yr_gr <- complete_four_yr_gr[!with(complete_four_yr_gr, is.na(complete_four_yr_gr$`English Learner Status`)),]

#Relabel category column
complete_four_yr_gr$`English Learner Status`[complete_four_yr_gr$`English Learner Status` == "ELL"] <- "English Language Learner"
complete_four_yr_gr$`English Learner Status`[complete_four_yr_gr$`English Learner Status` == "Non-ELL"] <- "Non-English Language Learner"
...
test <- complete_four_yr_gr_long[,c("District", "Year", "English Learner Status", "Variable")]
test2<-test[duplicated(test), ]

#Write CSV
write.table(
  complete_four_yr_gr_long,
  file.path(path_to_top_level, "data", "four_year_grad_rate_by_ell_2011-2015.csv"),
  sep = ",",
  row.names = F
)
